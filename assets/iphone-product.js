...
const colors={Black:['Чёрный','#303033'],Blue:['Голубой','#b9d3e2'],Burgundy:['Бургунди','#69333f'],Silver:['Серебристый','#d8d8d6'],White:['Белый','#f0ede7'],Natural:['Натуральный титан','#a69e91'],Pink:['Розовый','#eac6d6'],Teal:['Бирюзовый','#80aead'],Ultramarine:['Ультрамарин','#6476c2'],Lavender:['Лавандовый','#c8bddc'],Sage:['Шалфей','#acbba5'],Gold:['Золотистый','#e8ddbb'],Orange:['Оранжевый','#d87e4d']};
...
function showPhoto(p){
 if(!photo)return;
 const src=images[p.color],label=(data.model||p.model)+' — '+colorLabel(p.color);
 document.querySelector('#color-name').textContent=colorLabel(p.color);
 if(photo.dataset.color===p.color)return;
